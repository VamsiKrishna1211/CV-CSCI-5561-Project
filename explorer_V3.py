...
class SamEmbeddingModelWithFPN(torch.nn.Module):
    ...
    # Ensure comments reflect that FPN output keys need to match downstream usage.
     ...
     def forward(self, inputs):
         if isinstance(inputs, dict): inputs = inputs.get("pixel_values")
         if not isinstance(inputs, torch.Tensor): raise TypeError("Input must be Tensor or dict")

         # SAM Vision Encoder
         encoder_output = self.model(inputs, output_hidden_states=False)
         if hasattr(encoder_output, 'last_hidden_state'): features = encoder_output.last_hidden_state
         elif isinstance(encoder_output, torch.Tensor): features = encoder_output
         else: raise TypeError(f"Unexpected vision encoder output type: {type(encoder_output)}")

         # Reshape features: [B, N, C] -> [B, C, H, W]
         B, N, C = features.shape
         H, W = inputs.shape[-2], inputs.shape[-1] # Get input image H, W
         expected_H, expected_W = H // self.patch_size, W // self.patch_size
         if N != expected_H * expected_W:
             # Fallback: try sqrt if square
             H_W_sqrt = int(N**0.5)
             if H_W_sqrt * H_W_sqrt == N:
                 expected_H, expected_W = H_W_sqrt, H_W_sqrt
                 console_logger.warning(f"Inferred square grid H=W={H_W_sqrt} from patch count {N}. Input size was {H}x{W}.")
             else:
                 raise ValueError(f"Patch count {N} doesn't match expected grid {expected_H}x{expected_W} from input {H}x{W} and patch size {self.patch_size}.")

         features_reshaped = features.view(B, expected_H, expected_W, C).permute(0, 3, 1, 2)

         # Input to FPN requires a dictionary. Use a key that FPN recognizes based on its input stage.
         # Since it's the last layer, use a generic key like 'feat_last' or map to a standard stage name if appropriate.
         # Let's use '0' assuming FPN treats single input as first level.
         features_for_fpn = {'0': features_reshaped}

         # FPN Forward Pass
         fpn_output = self.fpn(features_for_fpn)
         # The keys here (e.g., '0', '1', '2', '3', 'pool') MUST match fpn_featmap_names in MaskRCNNLightning
         # print("DEBUG FPN keys:", fpn_output.keys()) # Uncomment for debugging
         return fpn_output
